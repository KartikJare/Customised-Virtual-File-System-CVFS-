            ...
            )
{
    ...
    {
        printf("Description : This command is used to read the data into file\n");

        printf("Usage : read File_Descriptor Size\n");

        printf("File_Descriptor : Its a value returned by create system call\n");

        printf("Size : Number of bytes taht you want to read\n");
    }
    // Add more options here
    else
    {
            ...
            )
{
    ...
    // Unable to write as there is no suffieient space
    if((MAXOPENEDFILES - uareaobj.UFDT[fd]->WriteOffset) < size)
    {
...
int read_file(
                ...
                char * data, // Address of the empty buffer
            ...
            )
{
    ...
    // Unable to read as there is no suffieient data
    if((MAXOPENEDFILES - uareaobj.UFDT[fd]->ReadOffset) < size)
    {
    ...
    while(1)
    {
        ...
        else if(iCount == 2)
        {
            ...
            {   
                ...
                {
                    printf("Error : Insuffieient space in the data block for the file\n");
                }
        ...
        else if(iCount == 3)
        {
            ...
            else if(strcmp(Command[0], "read") == 0)
            {
                ...
                {
                    printf("Error : Insuffieient space in the data block of the file\n");
                }
                else if(iRet == ERR_PERMISSION_DENIED)
                {
                    printf("Error : Unable to write as there is no read permission\n");
                }
                else if(iRet == ERR_INVALID_PARAMETER)
                {
                    printf("Error : Invalid parameters for the function\n");
                ...
                {
                    printf("Read opertion is successfull\n");
                    printf("Data from file is : %s\n",EmptyBuffer);
